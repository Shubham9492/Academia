...
int login(int client_socket, int type) {
    char *str;
    int status = 0, n;
    char recv_bf[20], login_id[60], password[40];
    str = "Enter the Username - ";
    write(client_socket, str, strlen(str));

    memset(recv_bf, 0, sizeof(recv_bf));
    read_line(client_socket, recv_bf, sizeof(recv_bf));
    my_strcpy(login_id, recv_bf);
    ...
    memset(recv_bf, 0, sizeof(recv_bf));
    read_line(client_socket, recv_bf, sizeof(recv_bf));
    my_strcpy(password, recv_bf);

    int fd;
    switch(type) {
        case ADMIN:
            if((strcmp(login_id, "admin") == 0) && (strcmp(password, "1234") == 0))
                status = 1;
            break;
        case FACULTY:
        case STUDENT:
    ...
    while(!status) {
        ...
        switch(atoi(recv_bf)) {
            ...
            case 4:
                ...
                break;
            case 5:
                updateStudent(client_socket, ACTIVE);
                break;
            case 6:
                updateStudent(client_socket, INACTIVE);
                break;
            case 7:
                updateStudent(client_socket, UPDATE);
                break;
            case 8:
                updateFaculty(client_socket);
                break;
            case 9:
    ...
    while(!status) {
        ...
        switch(atoi(recv_bf)) {
            ...
            case 3:
                ...
                break;
            case 4:
                updateCourse(client_socket, st.id);
                break;
            case 5:
...
int main(int argc, char **argv)
{
    ...
    printf("Listen socket creation successful\n");
            ...
            while(!status) {
                ...
                switch (atoi(recv_bf)) {
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        str = "Please enter the correct choice\n";
                        send(client_socket, str, strlen(str), MSG_MORE);
                }
            }
            close(client_socket);
