...
void getStudentDetails(int client_socket, struct student *st) {
    ...
    write(client_socket, str, strlen(str));
    read_line(client_socket, recv_bf, sizeof(recv_bf));
    my_strcpy(st->email, recv_bf);

    memset(recv_bf, 0, sizeof(recv_bf));
    str = "Address- ";
    write(client_socket, str, strlen(str));
    read_line(client_socket, recv_bf, sizeof(recv_bf));
    my_strcpy(st->address, recv_bf);
}

int addStudent(int client_socket) {
    struct student st, tmp;
    int fd;
    char roll_no[10] = "MT2023", buff[50];
    int n, status = 0;
    char *msg;
...
int updateStudent(int client_socket, int action) {
    struct student st;
    char *str;
    int fd;
    openStudentFile(&fd, O_RDWR);
